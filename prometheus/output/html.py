...
"""
    ...
    def tableDataRow(self, workload):
        ...
        self.appendTd(
            tr,
            workload.properMemoryRequest(),
            "memory",
            self.tdBgColor(len(workload.analyseMemoryRequest())),
        )  # something to notice -> set bgcolor
        self.appendTd(tr, workload.memoryLimit, "memory")
        self.appendTd(
            tr,
            workload.properMemoryLimit(),
            "memory",
            self.tdBgColor(len(workload.analyseMemoryLimit())),
        )  # something to notice -> set bgcolor
        ...
        self.appendTd(
            tr,
            workload.properCpuRequest(),
            "cpu",
            self.tdBgColor(len(workload.analyseCpuRequest())),
        )  # something to notice -> set bgcolor
        self.appendTd(tr, workload.cpuLimit, "cpu")
        self.appendTd(
            tr,
            workload.properCpuLimit(),
            "cpu",
            self.tdBgColor(len(workload.analyseCpuLimit())),
        )  # something to notice -> set bgcolor
