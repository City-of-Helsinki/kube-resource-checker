...
class Workload:
    ...
    def analyseMemoryRequest(self):
        ...
        if self.memoryRequest is None:
            ret.append(
                "Memory request has not set, proper value would be %sMi"
                % (math.ceil(properRequest))
            )
            return ret

        if self.memoryRequest < self.memoryAvg:
            ret.append(
                "Memory request is too small: request is %sMi, it should be %sMi"
                % (round(self.memoryRequest, 2), math.ceil(properRequest))
            )

        if self.memoryRequest > self.memoryMax and self.memoryRequest > (
            math.ceil(properRequest) * self.MEMORY_REQUEST_RESERVE_FACTOR_NOTIFY
        ):
            ret.append(
                "Memory request is exaggerated: request is %sMi, it should be %sMi"
                % (round(self.memoryRequest, 2), math.ceil(properRequest))
            )
    ...
    def analyseMemoryLimit(self):
        ...
        if self.memoryLimit is None:
            ret.append(
                "Memory limit has not set, proper value would be %sMi"
                % (math.ceil(self.properMemoryLimit()))
            )
            return ret
        ...
        if math.ceil(self.memoryLimit) <= math.ceil(self.memoryMax):
            ret.append(
                "Memory limit has reached: limit is %sMi. Is memory leak somewhere?"
                % (round(self.memoryLimit, 2))
            )
    ...
    def analyseCpuRequest(self):
        ...
        if self.cpuRequest is None:
            ret.append(
                "Cpu request has not set, proper value would be %sm"
                % (math.ceil(properRequest))
            )
            return ret
        ...
        if self.cpuRequest < self.cpuAvg:
            ret.append(
                "Cpu request is too small: request is %sm, it should be %sm"
                % (round(self.cpuRequest, 2), math.ceil(properRequest))
            )

        if self.cpuRequest > self.cpuMax and self.cpuRequest > (
            math.ceil(properRequest) * self.CPU_REQUEST_RESERVE_FACTOR_NOTIFY
        ):
            ret.append(
                "Cpu request is exaggerated: request is %sm, it should be %sm"
                % (round(self.cpuRequest, 2), math.ceil(properRequest))
            )
    ...
    def analyseCpuLimit(self):
        ...
        if self.cpuLimit is None:
            ret.append(
                "Cpu limit has not set, proper value would be %sm"
                % (math.ceil(self.properCpuLimit()))
            )
            return ret
        ...
        if math.ceil(self.cpuLimit) <= math.ceil(self.cpuMax):
            ret.append(
                "Cpu limit has reached: limit is %sm" % (round(self.cpuLimit, 2))
            )
    ...
    def properMemoryRequest(self):
        ...
        return self.roundProperToHuman(
            self.memoryAvg * self.MEMORY_REQUEST_RESERVE_FACTOR, 10
        )
    ...
    def properMemoryLimit(self):
        ...
        return self.roundProperToHuman(
            self.memoryMax * self.MEMORY_REQUEST_MAXIMUM_FACTOR, 50
        )
    ...
    def properCpuLimit(self):
        ...
        return self.roundProperToHuman(
            self.cpuMax * self.CPU_REQUEST_MAXIMUM_FACTOR, 100
        )
    ...
    def roundProperToHuman(self, value, minValue=0):
        ...
        )  # value is equal or over 1000, round up to 100
