...
class Workloads:
    ...
    def initWorkload(self, metric):
        ...
        if not workloadName in self.workloads:
            self.workloads[workloadName] = Workload(
                workloadName,
                metric["metric"]["namespace"],
                metric["metric"]["workload_type"],
            )
    ...
    def addMemoryRequest(self, metric):
        self.workloads[self.initWorkload(metric)].memoryRequest = float(
            metric["value"][1]
        )

    def addMemoryLimit(self, metric):
        self.workloads[self.initWorkload(metric)].memoryLimit = float(
            metric["value"][1]
        )
    ...
    def analyseMemoryBalance(self):
        ...
        print(
            f"Memory used (avg) vs requested : {math.ceil(usedAvg)} Mi/ {math.ceil(requested)} Mi"
        )
        print(
            f"Memory used (max) vs requested : {math.ceil(usedMax)} Mi/ {math.ceil(requested)} Mi"
        )
    ...
    def analyseCpuBalance(self):
        ...
        print(
            f"Cpu used (avg) vs requested : {math.ceil(usedAvg)} m/ {math.ceil(requested)} m"
        )
        print(
            f"Cpu used (max) vs requested : {math.ceil(usedMax)} m/ {math.ceil(requested)} m"
        )
